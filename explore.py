...
def check_LCS_verb_with_should_child(tree, entities, e1, e2):
...
# -----------------
# check pattern - SUGGESTION 1 REFINED
def check_LCS_verb_with_should_child_refined(tree, entities, e1, e2):

    # get head token for each gold entity
    tkE1 = tree.get_fragment_head(entities[e1]["start"], entities[e1]["end"])
    tkE2 = tree.get_fragment_head(entities[e2]["start"], entities[e2]["end"])

    # A set of verbs that are commonly associated with "advise" when modified by "should"
    advise_verbs = {
        "administer",
        "use",
        "take",
    ...
    if tkE1 is not None and tkE2 is not None:
        ...
        # Ensure the LCS is a verb and check its lemma
...
for f in listdir(datadir):
    ...
    for s in sentences:
        ...
        for p in pairs:
            ...
            match = check_LCS_verb_with_should_child(analysis, entities, id_e1, id_e2)

            # match = check_LCS_verb_with_should_child_refined(analysis, entities, id_e1, id_e2)
            ...
            if match is not None:
                ...
                print(
                    "entities:", entities[id_e1]["text"], "/", entities[id_e2]["text"]
                )
                print("ddi:", typ)
...
if countM == 0:
    ...
    for m in countP:
        for t in countMT:
            ...
            if k in countPT:
                if t == "advise":
                    if countPT[k] / countP[m] >= 0.3:
                        print(
                            f"P({t}|{m}) = {countPT[k]}/{countP[m]} = {countPT[k]/countP[m]}"
                        )
